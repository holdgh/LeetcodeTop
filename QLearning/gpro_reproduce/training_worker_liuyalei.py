...
class TrainingWorker:
    def __init__(self, config: dict):
        ...
        self.max_train_steps = config["training"]["max_train_steps"]
    ...
    def setup_model(self,local_rank):
        ...
        # 新增：GPU绑定逻辑（用local_rank指定当前进程的GPU）
        self.device = torch.device(f"cuda:{local_rank}")
        torch.cuda.set_device(self.device)  # 强制当前进程使用指定GPU
        print(f"当前进程绑定到GPU: {local_rank} (device: {self.device})")
        ...
        # # 初始化分布式进程组（替换原有dist.init_process_group逻辑）
        # dist.init_process_group(
        #     backend='nccl',  # A100推荐用nccl（比gloo快，若报错可换回gloo）
        #     rank=local_rank,
        ...
        # 验证当前进程的GPU（最终确认）
        print(f"Rank {self.rank} 绑定到GPU {torch.cuda.current_device()}，DeepSpeed引擎初始化完成")     
    ...
    def setup_zmq(self):
        ...
        self.data_receiver = None  # 先初始化为None
    ...
    def run(self):
        ...
        try:
            while not self.stop_event.is_set():
                # 新增：判断是否达到最大步数
                if train_step >= self.max_train_steps:
                    print(f"已达到最大训练步数 {self.max_train_steps}，自动停止训练")
...
def main():
    """主函数"""
    # 新增：解析DeepSpeed传递的local_rank参数
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--local_rank", type=int, default=-1)  # 接收local_rank
    ...
    # 关键：将local_rank传入setup_model
    worker.setup_model(local_rank=args.local_rank)
    print("=== GRPO训练进程 ===")
    print(f"数据端口: {config['communication']['data_port']}")
    print(f"DeepSpeed: 启用")
    # 修复：判断world_size>0后再打印
    world_size_info = f"{worker.world_size-1}" if worker.world_size > 0 else "0"
