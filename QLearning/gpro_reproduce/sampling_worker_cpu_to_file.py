...
from data_types import Gsm8kTasksDataset, Gsm8kZhTasksDataset
...
class SamplingWorker:
    def __init__(self, config: dict):
        ...
        # self.setup_zmq()  # 由于本地资源有限，不便同时运行采样进程和训练进程。拟采用
    ...
    def setup_data_loader(self):
        """初始化数据加载器"""
        train_dataset = Gsm8kZhTasksDataset(
    ...
    def setup_zmq(self):
        ...
        self.data_sender.bind(f"tcp://127.0.0.1:{self.zmq_data_port}")
    ...
    def sample_batch(self):
        ...
        # 计算旧策略概率分布
        batch_token_ids = torch.tensor([episode.whole_token_ids for episode in episodes], dtype=torch.long,
                                       device=self.device)
    ...
    def run(self):
        ...
        try:
            while not self.stop_event.is_set():
                ...
                sample_count += 1
                print(
                    f"{time.time() - last_sample_time:.2f}s, 采样{self.sample_batch_size}条数据, {self.num_questions_per_batch}个问题, 奖励为: {rewards}")
                last_sample_time = time.time()
                        ...
                        if not self.lora_adapter_dir:
                            ...
                            try:
                                if isinstance(self.old_policy_model, PeftModel):
                                    self.old_policy_model.load_adapter(self.lora_adapter_dir, adapter_name="default",
                                                                       is_trainable=False)
                                else:
                                    self.old_policy_model = PeftModel.from_pretrained(self.old_policy_model,
