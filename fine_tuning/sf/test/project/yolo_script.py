...
def InterVL_image_chat(base_url, prompt, input_image, question, mllm_model_path):
    ...
    print('mllm结果',result.choices[0].message.content)
    return result.choices[0].message.content
def InterVL_chat(base_url, prompt, question, mllm_model_path):
...
def Qwen_image_chat(base_url, prompt, input_image, question, mllm_model_path):

    print('测试点11')
    client = OpenAI(
        api_key="0",
        # base_url="http://192.168.0.194:6661/v1"
        # base_url="http://127.0.0.1:8000/v1"
        base_url=base_url
    )
    messages = [
...
def Qwen_chat(base_url, prompt, question, mllm_model_path):
...
def format_yolo_detections(input_image,detections, class_names):
    ...
    base_template = """你是一个面向3D打印场景的目标检测大师，具备精准分析图像缺陷的能力。
                        \n 职能:从给出的重点区域分析，该区域是否包含异常。
                        给定区域对应的异常类型可能会不正确，你需要先判断区域和类别是否正确，然后回答用户的问题。并且这个判断过程禁止输出。
                        \n 【禁止回答出】：异常在图中的大概位置，例如：某异常出现在图中的右边或者上边等。
                    """
    for det in detections:
        ...
        if class_name == "刮刀横/竖条纹":
            ...
            if x2-x1 < 200:
                ...
                class_name = "刮刀横条纹"
        # 添加到模板
        base_template += f"\n<box>{x1},{y1},{x2},{y2}</box>区域表现的异常类型可能是{class_name}"
