...
def train(
    ...
    # Load tokenizer
    tokenizer = AutoTokenizer.from_pretrained(base_model, verbose=False, padding_side="left", local_files_only=True,
    ...
    if data_dir is None:
        ...
        all_data = Dataset.from_json(os.path.join(data_dir, data_name))
    ...
    def tokenize_conversation_data(sample):
        ...
        if data_name == "ise-uiuc/Magicoder-Evol-Instruct-110K":
            ...
            image_path = "/nas_data/上飞/实验数据/" + images
            image_processor = AutoImageProcessor.from_pretrained(base_model, local_files_only=True)
        ...
        if assistant_only:
            ...
            result["labels"] = result["input_ids"].squeeze(0).clone()
            ...
            if image_path:
                ...
                flattened_image_features = pixel_values.view(1, -1)
                # 创建图像位置标记 (1表示有图像，0表示无)
                # image_flags = torch.ones_like(result.input_ids)
                combined_input = torch.cat((result.input_ids, flattened_image_features), dim=1)
                print("combined_input shape:", combined_input.shape)
                # 构造 image_flags
                image_flags = torch.ones_like(combined_input)
                image_flags[:, :-4096] = 0
                image_flags_reshaped = image_flags.view(1, 256, -1)
                print("image_flags_reshaped shape:", image_flags_reshaped.shape)
                # 对每个 patch 的标记进行汇总
                # 如果一个 patch 中有任何标记为 1，则该 patch 被认为是图像特征
                image_flags_downsampled = image_flags_reshaped.any(dim=2).int()
                print("image_flags_downsampled shape:", image_flags_downsampled.shape)
                result["image_flags"] = image_flags_downsampled.squeeze(0)

                # except Exception as e:
                #     print(f"图像处理失败: {e}")
                #     pixel_values = None
                #     image_flags = torch.zeros_like(result.input_ids)
                #     result["image_flags"]=image_flags.squeeze(0)
            else:
                pixel_values = None
                image_flags = torch.zeros_like(result.input_ids)

                result["image_flags"] = image_flags.squeeze(0)
            # print("image_flags shape:", image_flags.shape)
            if pixel_values is not None:
                result["pixel_values"] = pixel_values.squeeze(0)

            # result["pixel_values"]=image_tensors["pixel_values"]

            result["input_ids"] = result["input_ids"].squeeze(0)
            to_be_masked_indices = []
            to_be_masked_lst = []
            to_be_masked_lst += find_substrings_between_exclude(prompt, *USER_START_END[
                chat_template_name])  # Mask all user inputs
    ...
    # # 获取所有样本的 input_ids 长度
    # input_ids_lengths = [len(sample['input_ids']) for sample in train_data]
    # # 打印长度列表
    # print("Input IDs lengths:", input_ids_lengths)
    # image_flags_lengths = [len(sample['image_flags']) for sample in train_data]
    # # 打印长度列表
    # print("image_flags_lengths:", image_flags_lengths)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        cache_dir="/home/models/InternVL3-1B"  # 确保使用你的缓存路径
    )

    # print("model.config._name_or_path:",model.config._name_or_path)  # 应输出 "/home/models/InternVL3-1B"
    # print("model.__class__.__module__",model.__class__.__module__)
